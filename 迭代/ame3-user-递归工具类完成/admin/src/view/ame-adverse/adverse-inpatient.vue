    ...
    <Modal
      ...
      <div>
        <Form ref="userAddFormItem" :model="userAddFormItem" :rules="ruleUserAddFormItem" :label-width="80">
          <FormItem label="病区" prop="name">
            <Input v-model="userAddFormItem.name" style="width: 250px" placeholder="请输入病区名称"></Input>
          </FormItem>
          <FormItem label="描述" prop="textarea">
            <Input v-model="userAddFormItem.textarea" type="textarea" :autosize="{minRows: 2,maxRows: 5}" placeholder="请输入病区描述"></Input>
          </FormItem>
...
<script>
// var wardName = /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/
export default {
  name: 'adverse-inpatient',
  data () {
    const validateName = (rule, value, callback) => {
      if (!value) {
        return callback(new Error('病区名称不能为空'))
      }
      if (!isNaN(value)) {
        callback(new Error('请输入正确病区名字!'))
  ...
  methods: {
    ...
    delAllByIds () {
      ...
      for (let i = 0; i < allData.length; i++) {
        this.allIdData.push(allData[i].wardId)
      }
      // 发送请求
    ...
    getwardDataFromService () {
      ...
      this.axios.get('/ward/show', { params: page }).then((response) => {
    ...
    asyncOK () {
      this.$refs.modify_userAddFormItem.validate(valid => {
        if (valid) {
          this.axios.post('/ward/update', {
            wardId: this.userAddFormItem.wardId,
            name: this.userAddFormItem.name,
            wardDescribe: this.userAddFormItem.textarea
    ...
    modify_show (index) {
      ...
      this.userAddFormItem.textarea = this.userData[index].wardDescribe
      this.userAddFormItem.wardId = this.userData[index].wardId
    },
    modify_remove (index) {
      if (window.confirm(`确认删除${this.userData[index].name}吗？`)) {
        this.axios.get('/ward/del/' + this.userData[index].wardId)
          .then((response) => {
            this.getwardDataFromService()
          })
          .catch((error) => {
